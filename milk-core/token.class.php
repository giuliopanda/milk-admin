...
class Token {
     ...
     * @return void
     */
    public static function config_jwt($private_key = null, $public_key = null, $expiration = null) {
    ...
    public static function input($name) {
        return '<input type="hidden" name="'.self::get_token_name($name).'" value="'.self::get($name).'">';
    }
    ...
    public static function get($name) {
        return self::encrypt(json_encode([time(), self::get_token_key($name)]));
    }
     ...
     */
    public static function get_token_key($name) {
        return $name;
    }
     ...
     */
    public static function get_token_name($name) {
        $md5Name = md5($name);
    ...
    public static function check($name) {
        self::$last_error = '';
        $input_name = self::get_token_name($name);
        if (!isset($_REQUEST[$input_name])) {
            self::$last_error = 'input_not_found';
            return false;
        }
        $input_value = $_REQUEST[$input_name];
        return self::check_value($input_value, $name);
    }
     ...
     * @return boolean True if the token is valid, false otherwise
     */
    public static function check_value($token, $name) {
        self::$last_error = '';
        ...
        if (is_array($token_array) && count($token_array) > 0) {
            if (self::get_token_key($name) !== $token_array[1]) {
                self::$last_error = 'invalid_token';
                return false;
            }
            $time = $token_array[0];
     ...
     */
    public static function generate_jwt($user_id, $additional_data = []) {
        self::$last_error = '';
  ...
  /**
     ...
     */
    private static function sign_jwt($payload) {
        self::$last_error = '';
