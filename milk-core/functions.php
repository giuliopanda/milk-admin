 ...
 * // Works with arrays
 * $data = ['name' => 'John'];
 * $name = get_val($data, 'name'); // Returns 'John'
 * 
 * // Works with objects
 * $user = new User(); // Assuming User has a 'name' property
 * $name = get_val($user, 'name');
 * 
 * // Falls back to get_value() method if it exists
 * $value = get_val($someObject, 'custom_property');
 */
function get_val($class, $property) {
    if (is_array($class) && array_key_exists($property, $class)) {
        return $class[$property];
    } else if (property_exists($class, $property)) {
        return $class->$property;
    } else if (method_exists($class, 'get_value')) {
        return $class->get_value($property);
    } else {
        return null;
    }
}
 ...
 * with a to_mysql_array() method and falls back to default data when needed.
 *
 ...
 * // With a model object
 * $user = new User($id);
 * $data = to_mysql_array($user, $_POST);
 * 
 * // With a regular array
 * $data = to_mysql_array(['name' => 'John', 'age' => 30]);
 */
function to_mysql_array(mixed $obj, array $default_data = []) {
    $array_to_save = $default_data;
    if (method_exists($obj, 'to_mysql_array')) {
