 ...
 * 
 * Searching:
 * $results = Settings::search_by_value('localhost');
 * $keys = Settings::search_by_key('database');
 * 
 * Persistence:
...
class Settings
{
     ...
     */
    private static function sanitize_group_name(string $group): string
    {
     ...
     * 
     * Examples:
     * - get_file_path('database') → 'storage/database.json'
     * - get_file_path('user-prefs') → 'storage/user-prefs.json'
     * - get_file_path('admin@config') → 'storage/adminconfig.json'
     * 
     * @param string $group The group name
     * @return string The complete file path
     */
    private static function get_file_path(string $group): string
    {
        $clean_group = self::sanitize_group_name($group);
     ...
     * @return void
     */
    private static function load_group_data(string $group): void
    {
        $clean_group = self::sanitize_group_name($group);
        ...
        $file_path = self::get_file_path($group);
        
        // If file exists, load the data
        if (file_exists($file_path)) {
            $json_content = File::get_contents($file_path);
     ...
     */
    public static function search_by_value(mixed $search_value, ?string $group = null): array
    {
        $results = [];
        ...
        if ($group !== null) {
            ...
            if (isset(self::$data[$clean_group])) {
                foreach (self::$data[$clean_group] as $key => $value) {
                    if (self::value_contains($value, $search_value)) {
     ...
     * @return array<string, array<string, mixed>> Search results organized by group
     */
    public static function search_by_key(string $search_key, ?string $group = null): array
    {
        $results = [];
     ...
     * 
     * Examples:
     * - value_contains('hello world', 'WORLD') → true (case-insensitive)
     * - value_contains(['a', 'b', 'test'], 'TEST') → true (found in array)
     * - value_contains(123, '123') → true (numeric match)
     * - value_contains(['nested' => ['deep' => 'value']], 'value') → true (recursive)
     * 
     * @param mixed $value The value to search within
     * @param mixed $search_value The value to search for
     * @return bool True if the search value is found, false otherwise
     */
    private static function value_contains(mixed $value, mixed $search_value): bool
    {
        if (is_string($value)) {
            return stripos($value, $search_value) !== false;
        } elseif (is_array($value)) {
            foreach ($value as $item) {
                if (self::value_contains($item, $search_value)) {
                    return true;
                }
            }
    ...
    public static function save(): void
    {
        ...
        foreach (self::$modified_groups as $group => $is_modified) {
            if ($is_modified && isset(self::$data[$group])) {
                ...
                if (File::put_contents($file_path, $json_content) !== false) {
     ...
     * @return void
     */
    public static function remove_key(string $key, ?string $group = null): void
    {
        $group = $group ?: self::DEFAULT_GROUP;
        self::load_group_data($group);
     ...
     * @return void
     */
    public static function clear_group(?string $group = null): void
    {
        $group = $group ?: self::DEFAULT_GROUP;
        $clean_group = self::sanitize_group_name($group);
     ...
     */
    public static function has_key(string $key, ?string $group = null): bool
    {
        $group = $group ?: self::DEFAULT_GROUP;
        self::load_group_data($group);
