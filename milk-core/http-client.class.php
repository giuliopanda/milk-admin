...
class HttpClient
{
    ...
    public static function request($url, $options = [])
    {
        ...
        $ch = self::create_curl_handle($url, $options);
        if (!$ch) {
            return false;
        }
        ...
        return self::parse_response($response, $info);
    }
     ...
     */
    public static function execute_multi($requests)
    {
        self::$last_error = '';
        ...
        foreach ($requests as $key => $request) {
            ...
            if (!isset($request['url'])) {
                ...
                self::cleanup_multi_handles($multi_handle, $curl_handles);
                return false;
            }
        ...
        foreach ($curl_handles as $key => $ch) {
            ...
            if ($error || $response === false) {
                ...
                $responses[$key] = self::parse_response($response, $info);
            }
     ...
     */
    private static function create_curl_handle($url, $options)
    {
        $ch = curl_init();
        if (!$ch) {
        ...
        self::configure_http_method($ch, $options);
        
        // Request body configuration
        self::configure_body($ch, $options);
        
        // Headers configuration
        self::configure_headers($ch, $options);
     ...
     * @return void
     */
    private static function configure_http_method($ch, $options)
    {
     ...
     * @return void
     */
    private static function configure_body($ch, &$options)
    {
     ...
     * @return void
     */
    private static function configure_headers($ch, $options)
    {
     ...
     */
    private static function parse_response($response, $info)
    {
        ...
        $decoded_body = self::try_json_decode($body);
     ...
     */
    private static function parse_headers($headers_raw)
    {
        $headers = [];
     ...
     */
    private static function try_json_decode($body)
    {
        if (empty($body)) {
            return null;
        }
     ...
     * @return void
     */
    private static function cleanup_multi_handles($multi_handle, $curl_handles)
    {
     ...
     * ```
     */
    public static function set_default_option($key, $value)
    {
     ...
     * ```
     */
    public static function set_default_options($options)
    {
        if (is_array($options)) {
     ...
     * @return void
     */
    public static function reset_default_options()
    {
