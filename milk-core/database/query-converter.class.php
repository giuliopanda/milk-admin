...
class QueryConverter {
    ...
    public function convert($sql, $params = []) {
        ...
        switch ($this->target_db) {
            case 'postgres':
                $converted_sql = $this->convert_to_postgres($sql, $converted_params);
                break;
            case 'sqlite':
                $converted_sql = $this->convert_to_sqlite($sql, $converted_params);
                break;
            case 'mysql':
                $converted_sql = $this->convert_to_mysql($sql, $converted_params);
                break;
            default:
                $converted_sql = $sql;
                break;
     ...
     */
    private function convert_to_postgres($sql, &$params) {
        // 1. Converti i backtick in virgolette doppie
        $sql = $this->convert_quotes($sql, 'postgres');
        
        // 2. Converti i placeholder ? in $1, $2, ecc.
        $sql = $this->convert_placeholders($sql, 'postgres', $params);
        
        // 3. Converti LIMIT x,y in LIMIT y OFFSET x
        $sql = $this->convert_limit($sql, 'postgres');
        
        // 4. Converti le funzioni di data/ora
        $sql = $this->convert_date_functions($sql, 'postgres');
        
        // 5. Converti le funzioni di stringa
        $sql = $this->convert_string_functions($sql, 'postgres');
        
        // 6. Gestisci AUTO_INCREMENT -> SERIAL
        $sql = $this->convert_auto_increment($sql, 'postgres');
        
        // 7. Converti GROUP BY con colonne non aggregate (PostgreSQL è più restrittivo)
        $sql = $this->handle_group_by($sql, 'postgres');
     ...
     * @return string Query convertita
     */
    private function convert_to_sqlite($sql, &$params) {
        // 1. Converti i backtick in virgolette doppie
        $sql = $this->convert_quotes($sql, 'sqlite');
        ...
        // 4. Converti le funzioni di data/ora
        $sql = $this->convert_date_functions($sql, 'sqlite');
        
        // 5. Converti le funzioni di stringa
        $sql = $this->convert_string_functions($sql, 'sqlite');
        
        // 6. SQLite non ha un vero tipo BOOLEAN
        $sql = $this->convert_booleans($sql, 'sqlite');
     ...
     * @return string Query convertita
     */
    private function convert_to_mysql($sql, &$params) {
        // 1. Converti le virgolette doppie in backtick
        $sql = $this->convert_quotes($sql, 'mysql');
        
        // 2. Converti i placeholder numerati $1, $2 in ?
        $sql = $this->convert_placeholders($sql, 'mysql', $params);
        
        // 3. Converti LIMIT y OFFSET x in LIMIT x,y
        $sql = $this->convert_limit($sql, 'mysql');
        
        // 4. Converti le funzioni di data/ora
        $sql = $this->convert_date_functions($sql, 'mysql');
        
        // 5. Converti le funzioni di stringa
        $sql = $this->convert_string_functions($sql, 'mysql');
     ...
     */
    private function convert_quotes($sql, $target_db) {
        if ($target_db === 'mysql') {
     ...
     */
    private function convert_placeholders($sql, $target_db, &$params) {
     ...
     */
    private function convert_limit($sql, $target_db) {
        if ($target_db === 'postgres') {
     ...
     */
    private function convert_date_functions($sql, $target_db) {
        switch ($target_db) {
            case 'postgres':
                ...
                $sql = preg_replace_callback(
                    ...
                    function($matches) {
                        $field = $matches[1];
                        $format = $this->convert_date_format_to_postgres($matches[2]);
            ...
            case 'sqlite':
                ...
                $sql = preg_replace_callback(
                    ...
                    function($matches) {
                        $field = $matches[1];
                        $format = $this->convert_date_format_to_sqlite($matches[2]);
            ...
            case 'mysql':
                ...
                $sql = preg_replace_callback(
                    ...
                    function($matches) {
                        $field = $matches[1];
                        $format = $this->convert_date_format_to_mysql($matches[2]);
     ...
     */
    private function convert_string_functions($sql, $target_db) {
        switch ($target_db) {
            case 'postgres':
                // CONCAT -> ||
                $sql = preg_replace_callback(
     ...
     * @return string Query convertita
     */
    private function convert_auto_increment($sql, $target_db) {
        if ($target_db === 'postgres') {
     ...
     * @return string Query convertita
     */
    private function handle_group_by($sql, $target_db) {
     ...
     * @return string Query convertita
     */
    private function convert_booleans($sql, $target_db) {
        if ($target_db === 'sqlite') {
            // TRUE -> 1, FALSE -> 0
     ...
     */
    private function convert_date_format_to_postgres($format) {
        $conversions = [
            '%Y' => 'YYYY',
            '%y' => 'YY',
            '%m' => 'MM',
     ...
     */
    private function convert_date_format_to_sqlite($format) {
     ...
     */
    private function convert_date_format_to_mysql($format) {
        $conversions = [
            'YYYY' => '%Y',
            'YY' => '%y',
            'MM' => '%m',
