            ...
            <tr>
                <?php foreach ($info as $key => $header) : ?>
                    ...
                    <th data-attrid="th.<?php _p(str_replace(' ','_', $key)); ?>" 
                        scope="col" <?php Template::add_attrs($table_attrs, 'th.'.str_replace(' ','_', $key), 'th', 'th'); ?>>
                    ...
                    <?php else : ?>
                            ...
                            <?php else : ?>
                                <div class="d-flex"><span class="me-2 table-black-link"><?php _p($header['label']); ?> </span></div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                    </th>
                ...
                <?php foreach ($rows as $row) : ?> 
                    <tr <?php Template::add_attrs($table_attrs, 'tr'); ?>>
                        <?php foreach ($info as $col_name => $header) : ?>
                            ...
                            <td <?php Template::add_attrs($table_attrs, 'td.'.str_replace(' ','_', $col_name), 'td', 'td'); ?>><?php echo get_val($row, $col_name); ?></td>
                        <?php endforeach; ?>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        ...
        <?php if ($page_info['footer'] ?? false && $footer_row > 0) : ?>
            <tfoot <?php Template::add_attrs($table_attrs, 'tfoot'); ?>>
                <tr <?php Template::add_attrs($table_attrs, 'tfoot.tr'); ?>>
                    <?php foreach ($info as $col_name => $header) : ?>
                        <?php if ($header['type'] == 'hidden') continue; ?>
                        <td  data-attr="<?php _p('tfoot.td.'.str_replace(' ','_', $col_name)); ?>" <?php Template::add_attrs($table_attrs, 'tfoot.td.'.str_replace(' ','_', $col_name), null, ['id' =>  _r( $table_id."_".str_replace(' ','_', $col_name) )]); ?>><?php get_val($footer_row, $col_name); ?></td>
                    <?php endforeach; ?>
                </tr>
            </tfoot>
        <?php endif; ?>
