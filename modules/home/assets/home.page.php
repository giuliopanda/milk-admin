...
foreach ($dbs as $dbKey => $db) {
    ...
    if ($db) {
        ...
        if ($db->type == 'mysql') {
            ...
            $version = $db->get_row('SELECT VERSION() as version');
            ...
            $storageEngine = $db->get_row("SHOW VARIABLES LIKE 'storage_engine'");
            $dbVersion = $storageEngine ? $storageEngine->Value : 'Unknown';
        } else {
            // SQLite version
            $version = $db->get_row('SELECT sqlite_version() as version');
