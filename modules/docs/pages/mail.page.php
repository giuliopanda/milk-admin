...
Get::mail()->load_template(MILK_DIR.'/modules/my_module/mails/email_template.php', [
    'name' => 'John Doe',
    'user' => $user_object,
...
// Error handling
if (Get::mail()->get_last_error()) {
    echo Get::mail()->get_last_error();
}</code></pre>
...
<?php echo htmlentities("// Additional PHPMailer parameters (optional)\n\$this->mail->Priority = 1; // 1 = High, 3 = Normal, 5 = Low\n\$this->mail->CharSet = 'UTF-8';"); ?>
    ...
    <h3 class="mt-4">load_template($path, $vars = [])</h3>
    <p>Loads a PHP template for the email. Automatically searches in the customizations path before the original path.</p>
    <ul>
        <li><code>$path</code>: template file path</li>
        <li><code>$vars</code>: associative array of variables to pass to the template</li>
    </ul>
    
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php"><?php echo htmlentities("Get::mail()->load_template('modules/user/mails/welcome.php', [\n    'username' => 'Mario Rossi',\n    'activation_link' => 'https://example.com/activate?token=123'\n]);"); ?></code></pre>
    ...
    <h3 class="mt-4">has_error()</h3>
    <p>Checks if there were any errors during sending.</p>

    <h3 class="mt-4">get_error()</h3>
...
<p>Best regards,<br>The Team</p>\n<?php\n\$this->mail->Body = ob_get_clean();\n\$this->mail->AltBody = strip_tags(\$this->mail->Body);\n\$this->mail->Subject = 'Monthly Report - ' . \$month;\n\n// Send with attachment\n\$mail = Get::mail();\n\$mail->load_template('modules/reports/mails/monthly_report.php', [\n    'recipient_name' => 'Manager',\n    'month' => 'October 2024'\n])\n->to('manager@example.com')\n->addAttachment('/path/to/report.pdf', 'Report_October_2024.pdf')\n->send();"); ?></code></pre>
    ...
    <ul>
        ...
        <li>Always handle errors with get_last_error()</li>
    </ul>
