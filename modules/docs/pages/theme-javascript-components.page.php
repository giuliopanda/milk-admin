...
<pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">protected function json_permission_denied( $custom_message = ''): void {
...
<h3>4. Router Usage Example</h3>
<p>In the router, when a permission issue occurs in an action:</p>

<pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">protected function action_json_related_tables_save() {
   
    // Permission check
    Permissions::check_json('my_class.edit');
...
<ol>
    ...
    <li>If permissions are denied, <code>json_permission_denied()</code> is called</li>
