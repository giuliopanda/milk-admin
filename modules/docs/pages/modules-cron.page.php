...
Theme::set('header.breadcrumbs', '<a class="link-action" href="?page=jobs">Jobs</a> Help');
?>
...
$scheduler = $jobs_contract::create_scheduler()
    ->set_minutes(0)
    ->set_hours(9)
    ->set_day_of_week('1-5'); // Monday to Friday
...
$jobs_contract::register(
   ...
   <p>Each module's controller class (extending AbstractController) can implement these corresponding methods:</p>
   <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">
class MyModuleController extends \MilkCore\AbstractController {
   ...
   <p>The recommended place to add your custom cron jobs is in the <code>customizations/functions.php</code> file:</p>
...
$scheduler = $jobs_contract::create_scheduler()
    ...
    ->set_hours(14)
    ->set_day_of_month(15)
    ->set_month('1,4,7,10'); // January, April, July, October
...
$jobs_contract::register(
   ...
   <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">
function my_job_function($metadata) {
    try {
        // Your job logic here
   ...
   <h2>Conclusion</h2>
   <p>The Cron module provides a flexible way to schedule recurring tasks in your application. Add your custom cron jobs in the <code>customizations/functions.php</code> file using the <code>Get::make('jobs_contract')</code> service, or create custom modules for more complex scheduling needs.</p>
</div>