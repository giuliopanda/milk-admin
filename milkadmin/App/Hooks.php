...
class Hooks
{
        ...
        if (array_key_exists($name, self::$functions)) { 
            ...
            foreach (self::$functions[$name] as $function) {
                if (is_callable($function['fn'])) {
                    $new_args = self::completeArgs($function['fn'], $args, $value);
     ...
     */
    public static function getHookRegistrations($name): array {
        $name = _raz($name);
        if (array_key_exists($name, self::$functions)) {
     ...
     */
    static private function completeArgs($function, $args, $value) {
