...
<div class="bg-white p-4">
    ...
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">Get::themePlugin('example', ['hello' => 'first button', 'alert' => 'Hello World 1']);</code></pre>
    </p>
...
<pre class="pre-scrollable border p-2" class="text-bg-gray"><code class="language-php">echo Get::themePlugin('example', ['hello' => 'first button', 'alert' => 'Hello World 1']);
echo Get::themePlugin('example', ['hello' => 'Second button', 'alert' => 'Hello World 2']);</code></pre>

<h1 class="mt-4">Table example</h1>
<p>Tables are one of the fundamental elements for data visualization, so the system has a specific plugin to handle tables. You can learn more about this topic <a href="<?php echo Route::url('?page=docs&action=Framework/DynamicTable/modellist-table-p1'); ?>">here</a></p>
<?php
 $info = [
...
?><h4>Dynamic Table</h4>
<pre><code class="language-php">echo Get::themePlugin('table', ['info' => $info, 'rows' => $rows, 'page_info' => $page_info]);</code></pre>
<?php
echo Get::themePlugin('table', ['info' => $info, 'rows' => $rows, 'page_info' => $page_info]);

?><h4>Static table</h4>
<pre><code class="language-php">echo Get::themePlugin('staticTable', ['rows' => $rows]);</code></pre>
<?php
