...
if ($response['status_code'] === 200) {
    ...
    <h3 class="mt-4">executeMulti($requests)</h3>
...
foreach ($responses as $key => $response) {
    ...
    <h3 class="mt-4">setDefaultOption($key, $value)</h3>
    <p>Sets a single default option that will be applied to all requests.</p>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">HttpClient::setDefaultOption('timeout', 60);
HttpClient::setDefaultOption('user_agent', 'MyApp/1.0');</code></pre>

    <h3 class="mt-4">setDefaultOptions($options)</h3>
    ...
    <h3 class="mt-4">getDefaultOptions()</h3>
    <p>Returns the current default options.</p>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">$current_options = HttpClient::getDefaultOptions();
print_r($current_options);</code></pre>

    <h3 class="mt-4">resetDefaultOptions()</h3>
    <p>Resets all options to their original default values.</p>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">HttpClient::resetDefaultOptions();</code></pre>

    <h2 class="mt-4">Error Handling</h2>

    <h3 class="mt-4">getLastError()</h3>
...
if (!$response) {
    echo "Error: " . HttpClient::getLastError();
}</code></pre>
...
$result = HttpClient::delete('https://api.example.com/users/123');</code></pre>
    ...
    <ul>
        ...
        <li>Use <code>getLastError()</code> to handle errors gracefully</li>
        <li>Set appropriate timeouts based on expected response times</li>
        <li>Use <code>executeMulti()</code> for multiple concurrent requests to improve performance</li>
    ...
    <ul>
        ...
        <li>Multiple requests using <code>executeMulti()</code> can significantly improve performance</li>
    </ul>
</div>