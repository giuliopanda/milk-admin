...
<div class="bg-white p-4">
    ...
    <h4 class="mt-4">getContents($file_path)</h4>
...
if ($content !== false) {
    ...
    echo "Read failed: " . File::getLastError();
}
...
if ($config_json !== false) {
    ...
    <h4 class="mt-4">putContents($file_path, $data)</h4>
...
if (File::putContents('log.txt', $log_entry)) {
    echo "Log entry written successfully";
} else {
    echo "Failed to write log: " . File::getLastError();
}

// Writing JSON data
$data = ['user' => 'john', 'timestamp' => time()];
$json = json_encode($data, JSON_PRETTY_PRINT);
if (File::putContents('user_data.json', $json)) {
    echo "User data saved";
}</code></pre>

    <h4 class="mt-4">appendContents($file_path, $data)</h4>
...
if (File::appendContents('app.log', $log_entry)) {
...
if (File::appendContents('items.txt', $new_item)) {
    echo "Item added to list";
}

// CSV data appending
$csv_row = "John,Doe,30,Engineer\n";
if (File::appendContents('users.csv', $csv_row)) {
    echo "User data appended to CSV";
}</code></pre>

    <h4 class="mt-4">waitLock($fp, $exclusive = true)</h4>
...
if (!File::putContents('readonly.txt', 'data')) {
...
foreach ($files as $file) {
    if (!File::putContents($file, 'test data')) {
        echo "Failed to write $file: " . File::getLastError() . "\n";
    }
}</code></pre>
...
// Process A
$content_a = File::getContents('shared_data.txt'); // Shared lock

// Process B (simultaneous)
$content_b = File::getContents('shared_data.txt'); // Shared lock (allowed)

// Process C wants to write
if (File::putContents('shared_data.txt', 'new data')) {
...
function safeLog($message) {
    ...
    if (File::appendContents('app.log', $log_entry)) {
        ...
        error_log("Logging failed: " . File::getLastError());
        return false;
    }
}

// Usage from different processes
safeLog("User login: john@example.com");
safeLog("Database connection established");
safeLog("Cache cleared by admin");</code></pre>
...
function loadConfig($config_file) {
    $json = File::getContents($config_file);
    if ($json === false) {
        echo "Config load failed: " . File::getLastError();
        return [];
    }
...
function saveConfig($config_file, $config) {
    ...
    if (File::putContents($config_file, $json)) {
        echo "Configuration saved successfully";
        return true;
    } else {
        echo "Config save failed: " . File::getLastError();
        return false;
    }
}

// Usage
$config = loadConfig('app_config.json');
$config['last_updated'] = time();
saveConfig('app_config.json', $config);</code></pre>
...
function collectUserData($user_id, $action) {
    $data = [
        'timestamp' => time(),
        'user_id' => $user_id,
        'action' => $action,
    ...
    if (File::appendContents('user_tracking.csv', $csv_line)) {
        return true;
    } else {
        error_log("Tracking failed: " . File::getLastError());
        return false;
    }
}
...
function processTrackingData() {
    $csv_data = File::getContents('user_tracking.csv');
    if ($csv_data !== false) {
        $lines = explode("\n", trim($csv_data));
...
// Usage
collectUserData(123, 'login');
collectUserData(456, 'purchase');
processTrackingData();</code></pre>
...
function batchFileOperations() {
    ...
    foreach ($operations as $op) {
        ...
        if ($action === 'write') {
            $result = File::putContents($file, $data);
        } else {
            $result = File::appendContents($file, $data);
        }
        
        if ($result) {
            $success_count++;
        } else {
            $errors[] = "$action on $file: " . File::getLastError();
        }
    }
