...
$users = $db->getResults("SELECT * FROM users WHERE active = 1");
    </code></pre>
...
$analytics = $db2->getResults("SELECT * FROM analytics_data WHERE date >= ?", ['2024-01-01']);
    </code></pre>
...
while ($row = $result->fetch_array()) {
    ...
    <h4>fetchAssoc()</h4>
    <p>Fetches a result row as an associative array. Alias of fetchArray().</p>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">
...
$db = Get::db();
$users = $db->getResults("SELECT * FROM app_users WHERE active = 1");

// Use secondary database (SQLite)  
$db2 = Get::db2();
$cache = $db2->getResults("SELECT * FROM local_cache WHERE expires > ?", [time()]);
...
if ($db->type === 'mysql') {
    foreach ($db->nonBufferedQuery("massive_table") as $key => $row) {
...
$db = Get::db();
$results = $db->getResults("SELECT * FROM users WHERE created_at > ?", ['2024-01-01']);
    </code></pre>
