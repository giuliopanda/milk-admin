...
<div class="bg-white p-4">
   ...
   console.log(data);
    ...
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">if (Token::checkValue($_POST['token'], 'form_name')) { //... }</code></pre> 

    <h4 class="mt-4">getTokenKey($name)</h4>
    <p>Returns the token key generated from the provided name.</p>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">$tokenKey = Token::getTokenKey('form_name');</code></pre>

    <h4 class="mt-4">getTokenName($name)</h4>
    <p>Returns the name of the variable where the token is stored.</p>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">$tokenName = Token::getTokenName('form_name');</code></pre>
    ...
    <h4 class="mt-4">checkValue($token, $name)</h4>
    <p>Verifies if the CSRF token value is correct.</p>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">$isValid = Token::checkValue($token, 'form_name');</code></pre>

    <h2 class="mt-5">JWT Token Functions</h2>

    <h4 class="mt-4">configJwt($private_key, $public_key, $expiration)</h4>
    <p>Configures keys and expiration for the JWT system. If parameters are not provided, global configurations are used.</p>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">Token::configJwt($private_key, $public_key, 3600); // 1 hour</code></pre>

    <h4 class="mt-4">generateKeyPair()</h4>
    <p>Generates a public/private key pair for JWT authentication with 2048-bit RSA algorithm.</p>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">$keys = Token::generateKeyPair();
if ($keys) {
    $private_key = $keys['private_key'];
    $public_key = $keys['public_key'];
}</code></pre>

    <h4 class="mt-4">generateJwt($user_id, $additional_data)</h4>
...
$token = Token::generateJwt(123, $additional_data);</code></pre>

    <h4 class="mt-4">verifyJwt($token)</h4>
    <p>Verifies a JWT token and returns its payload if valid. Automatically checks expiration and token integrity.</p>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">$payload = Token::verifyJwt($token);
if ($payload) {
