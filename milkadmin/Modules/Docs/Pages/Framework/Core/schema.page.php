...
$schema->drop();</code></pre>
    ...
    <h4 class="mt-4">getFieldDifferences()</h4>
    <p>Returns an array containing the differences between the current database schema and the new schema definition after calling <code>create()</code> or <code>modify()</code>.</p>
    <p>This method is useful for tracking what changes were made to the table structure, including:</p>
    <ul>
        <li>Fields that were added</li>
        <li>Fields that were modified (type, length, default value, etc.)</li>
        <li>Fields that were removed</li>
    </ul>
    <p><strong>Example:</strong></p>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">$schema = Get::schema('#__users');
$schema->id()
    ->string('username', 150)  // Changed from 100 to 150
    ->string('email', 100)     // New field
    ->modify();

$differences = $schema->getFieldDifferences();
// Returns array with details about what changed</code></pre>
