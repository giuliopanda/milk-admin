...
// Returns: ['onlyActive']</code></pre>
    ...
    <h2 class="mt-4">Iterating Records with rows()</h2>

    <p>By default, a <code>foreach</code> on a model returns <code>$this</code> on every iteration — the same instance with the internal cursor moved forward. This means <code>$socio</code> and <code>$soci</code> are the same object.</p>

    <p>The <code>rows()</code> method solves this by yielding a <strong>new, independent model instance</strong> for each record, so each iteration gives you a proper isolated object.</p>

    <h3>Usage</h3>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">$soci = $model->select('MATRIC, NOME, NOME_SPLIT, COGNOME_SPLIT')->limit(100)->getResults();

foreach ($soci->rows() as $socio) {
    // $socio is a new model instance containing only this record
    echo $socio->NOME;
    echo $socio->MATRIC;
}</code></pre>
    ...
    <div class="alert alert-warning">
        <strong>Not performant for large datasets.</strong> Each iteration instantiates a new model object via <code>new static()</code>. For 1000 records this means 1000 object instantiations, each running the full constructor (configure, extensions, attribute scanning).
    </div>

    <p><strong>Use <code>rows()</code> when:</strong></p>
    <ul>
        <li>You need to call model methods on each individual record (e.g. <code>save()</code>, <code>validate()</code>, relationships)</li>
        <li>You need an isolated instance to pass to other services</li>
        <li>The dataset is small (hundreds of records, not thousands)</li>
    </ul>

    <p><strong>Avoid <code>rows()</code> when:</strong></p>
    <ul>
        <li>You only need to read field values — use a plain <code>foreach ($soci as $_)</code> with <code>$soci->FIELD</code> instead</li>
        <li>The dataset is large</li>
    </ul>

    <h3>Difference from plain foreach</h3>
    <pre class="pre-scrollable border p-2 text-bg-gray"><code class="language-php">// Plain foreach: $socio === $soci (same object, cursor moves)
foreach ($soci as $socio) {
    echo $socio->NOME;  // works, but $socio is $soci
}

// rows(): $socio is a NEW independent model instance
foreach ($soci->rows() as $socio) {
    echo $socio->NOME;  // works, and $socio !== $soci
    $socio->save();     // safe: operates only on this record
}</code></pre>
