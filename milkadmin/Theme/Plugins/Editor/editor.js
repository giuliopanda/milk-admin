...
class TrixEditorManager {
    ...
    /**
     * Static method to initialize Editor from JSON config
     * @param {string} containerId - The ID of the container element
     * @returns {string|null} - The container ID or null if failed
     */
    static initFromConfig(containerId) {
        const container = document.getElementById(containerId);
        if (!container) {
            console.warn(`TrixEditor: Container with id "${containerId}" not found`);
            return null;
        }

        // Check if already initialized
        if (container.dataset.editorInitialized === 'true') {
            return containerId;
        }

        // Get config ID from data attribute
        const configId = container.dataset.editorConfig;
        if (!configId) {
            console.warn(`TrixEditor: No config ID found for container "${containerId}"`);
            return null;
        }
        ...
        if (!configElement) {
            console.warn(`TrixEditor: Config element "${configId}" not found`);
            return null;
        }
        ...
        try {
            ...
            console.error(`TrixEditor: Failed to parse config for "${containerId}"`, e);
            return null;
        }

        // Handle callbacks - they are stored as strings, need to be evaluated
        const callbacks = {};
        ['onChange', 'onBlur', 'onFocus', 'onFileAccept', 'onAttachmentAdd', 'onAttachmentRemove'].forEach(callbackName => {
            if (config[callbackName] && typeof config[callbackName] === 'string') {
                try {
                    // Try to get the function from window scope
                    callbacks[callbackName] = window[config[callbackName]] || null;
                } catch (e) {
                    callbacks[callbackName] = null;
                }
            } else {
                callbacks[callbackName] = config[callbackName] || null;
            }
        });

        // Create editor instance (use the actual container ID, not the one from config)
        const editorId = window.editor.createEditor({
            containerId: containerId,  // Use the actual container ID from the DOM
            value: config.value || '',
        ...
        // Set name attribute if specified
        if (config.name && editorId) {
            setTimeout(() => {
                const instance = window.editor.instances.get(editorId);
                if (instance && instance.hiddenInput) {
                    instance.hiddenInput.setAttribute('name', config.name);
                }
            }, 100);
        }
    ...
    /**
     * Static method to initialize all Editor elements in a container
    ...
    static initAll(container = document) {
        ...
        if (!containerElement) {
            console.warn('TrixEditor: Container not found');
            return;
        }

        // Find all divs with editor config
        const elements = containerElement.querySelectorAll('div[data-editor-config]');
        elements.forEach(element => {
            if (element.dataset.editorInitialized !== 'true') {
                TrixEditorManager.initFromConfig(element.id);
            }
        });
    }
...
// This MutationObserver watches for new Editor elements added to the DOM
        ...
        mutations.forEach((mutation) => {
            mutation.addedNodes.forEach((node) => {
                ...
                if (node.nodeType === 1) {
                    ...
                    if (node.matches && node.matches('div[data-editor-config]')) {
                        if (node.dataset.editorInitialized !== 'true') {
                            TrixEditorManager.initFromConfig(node.id);
                        }
                    }
                    // Check for editor containers within the added node
                    if (node.querySelectorAll) {
                        const editors = node.querySelectorAll('div[data-editor-config]');
                        editors.forEach(editor => {
                            if (editor.dataset.editorInitialized !== 'true') {
                                TrixEditorManager.initFromConfig(editor.id);
                            }
                        });
                    }
                }
